                        ...
                        <div class="row g-0 align-items-center">
                            <div class="col-md-4 d-flex justify-content-center align-items-center bg-light p-3">
                                <img src="{{ asset('storage/foto_user/' . $jobsAll->perusahaan->foto) }}"
                                    class="img-fluid rounded-circle border" alt="Logo Perusahaan"
                            ...
                            <div class="col-md-8">
                                <div class="card-body p-4">
                                    ...
                                    <ul class="list-unstyled small mb-3">
                                        ...
                                        <li><i
                                                class="bi bi-currency-dollar me-2"></i>Rp{{ number_format($jobsAll->gaji_minimum) }}
                                            - Rp{{ number_format($jobsAll->gaji_maximum) }}</li>
                                    </ul>

                                    <div class="d-flex flex-wrap gap-2 mb-3">
                                        @foreach ($jobsAll->kategori_lowongan as $kategori)
                                            <span
                                                class="badge bg-light text-dark border rounded-pill">{{ $kategori }}</span>
                                        @endforeach
                                        {{-- @foreach (json_decode($jobsAll->kategori_lowongan) ?? [] as $kategori)
                                            <span
                                                class="badge bg-light text-dark border rounded-pill">{{ $kategori }}</span>
                                        @endforeach --}}
                                    </div>

                                    <a href="{{ route('statusjobs', $jobsAll->id) }}"
