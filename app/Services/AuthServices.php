    ...
    public static function updateProfile(array $data)
    {
        try {
            ...
            if (!empty($validated['foto'])) {
                if ($user->foto && file_exists(storage_path('app/public/storage/foto_user' . $user->foto)) && $user->foto != 'deflaut_user.png') {
                    FileService::delete('app/public/storage/foto_user' . $user->foto);
                }
            ...
            if (!empty($validated['file_cv'])) {
                if ($detailUser->file_cv && file_exists(storage_path('app/public/storage/file_cv' . $detailUser->file_cv))) {
                    FileService::delete('app/public/storage/file_cv' . $detailUser->file_cv);
                }
