...
class LowonganService
{
    ...
    public function addNewLowongan(array $data): RedirectResponse
    {
        ...
        if ($validator->fails()) {
            return redirect()->back()->with('fails_lowongan', 'pastikan semua fild terisi');
        }
    ...
    {
        try {
            return lowongan_applys::where('lowongan_id', $lowoganId)
                ->where('pelamar_id', Auth::user()->id)
                ->first();
        } catch (\Exception $e) {
            return Redirect::back()->withErrors(['error' => 'Gagal mengambil daftar pelamar: ' . $e->getMessage()]);
        }
